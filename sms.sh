...
while true; do
  ...
  for i in {1..10}; do
    printf "\r%c Waiting for new incoming messages... " "${spin:i%${#spin}:1}"
    sleep 0.1
  done
	...
	#find the last received message, and grep only the body of it.
 	...
 	#disable loopback
        if [ "$old_rep" != "$body" ]; then
            ...
            echo ""
            echo "{<--} Incoming message N^$new_counter form $addr"
            echo "{type}:		inbound"
            echo "{body}:		$body"
            echo ""
            
            #generate chatgpt reply
            raw_rep="$(sgpt --chat "$addr-$(date +%Y-%m-%d)" "\"$body\" ")"
            rep=$(echo $raw_rep | sed 's/ /\\ /g' | sed "s/'/\\\'/g" | sed 's/(/- -/; s/)/- -/g')
            ...
            adb shell am start -a android.intent.action.SENDTO -d sms:"$addr" --es sms_body "${rep}" --ez exit_on_sent false;
            sleep 0.5;
